...
class HTMlLogger(metaclass=Singleton):
    filepath = str(Path(__file__).parent.parent.parent) + "/reports/latesttree/report_" + str(
    ...
    def assert_testcase_log(self, log):
        ...
        tlog = """
            ...
            self.parentId) + """'><img src="img/pass_4.png"/>PASS</td><td class="data">""" + str(
    ...
    def assert_step_log(self, status, log):
        ...
        tLog = """
                      <tr row-id='""" + str(self.rowid) + """' parent-id='""" + str(self.parentId) + """'>
                        <td>""" + log + """</td><td class="data"><img src="img/pass_4.png"/>&nbsp """ + status + """</td><td class="data">""" + str(
            time.strftime("%H:%M:%S")) + """</td>
                      </tr>
                    """
        self.f.writelines(tLog)
    ...
    def assert_step_fail_log(self, driver, log):
        ...
        snapPath = snapPath.replace("latesttree/report_", "failureimage/snap_")
        ...
        tLog = """
                      <tr row-id='""" + str(self.childId) + """' parent-id='""" + str(self.parentId) + """'>
                        <td>""" + log + """</td><td class="fail" id="fail" parentid='""" + str(
            self.parentId) + """'><img src="img/fail.png"/>&nbsp FAIL</td><td class="data">""" + str(
            time.strftime("%H:%M:%S")) + """</td>
                      </tr>
                      <tr row-id='""" + str(self.childId + 1) + """' parent-id='""" + str(self.childId) + """'>
                        <td colspan="3"><img src='../failureimage/""" + strP + """' width="25%" height="25%"/></td>
                      </tr>
                    """
        self.f.writelines(tLog)
        self.f.close()
